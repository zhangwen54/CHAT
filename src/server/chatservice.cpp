...
void ChatService::reg(const TcpConnectionPtr &conn, json &js, Timestamp time)
{
    ...
    if (state)
    {
        ...
        response["errno"] = 0;
        response["id"] = user.getId();
        conn->send(response.dump()); //给客户端发送response回去
    }
    else
    {
        // 注册失败
...
void ChatService::oneChat(const TcpConnectionPtr &conn, json &js, Timestamp time)
{
    ...
    /*将下面块中的代码注释掉，当一对一聊天时，聊天信息会经过redis发布-订阅中转，如果不注释掉，聊天时聊天信息会直接中转。*/
    /*
    {
        lock_guard<mutex> lock(_connMutex);
...
/*注意：该函数将作为回调函数，在该ChatService类初始化时注册到redis.cpp中，该回调函数属于ChatService类的一个成员，
且该函数使用了ChatService成员——connMutex锁变量和_userConnMap变量，而一般函数在redis.cpp中是无法使用这两个
变量的，只能通过redis.cpp中调用ChatService注册过去的回调函数，才能实现。另外，ChatService的handleRedisSubscribeMessag函数
只有在特定情况下才执行，而这个特定情况ChatService自己不知道，需要由redis.cpp决定。所以注册到redis.cpp中，由
redis.cpp决定何时调用该函数。！！！！*/
// 从redis消息队列中获取订阅的消息
